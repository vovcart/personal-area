...
//Solution nr-1
  def almostIncreasingSequence1(sequence: Array[Int]): Boolean = {
    (1 until sequence.length)
      .map(i => if (sequence(i - 1) < sequence(i)) 0 else 1)
      .sum <= 1 &&
    (2 until sequence.length)
      .map(i => if (sequence(i - 2) < sequence(i)) 0 else 1)
      .sum <= 1
  }
  ...
  def almostIncreasingSequence3(sequence: Array[Int]): Boolean = {
    ...
    (sequence, sequence.drop(2)).zipped
